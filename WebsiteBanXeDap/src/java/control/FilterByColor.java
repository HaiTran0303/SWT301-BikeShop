    ...
    throws ServletException, IOException {
        ...
        if (checkbox.length != 0) {
            for (String x : checkbox) {
                ...
                if (x.equals("1")) {
                    List<Product> list = d.getProductByColor("Black");
                    addProductToList(listA, list);

                }
                if (x.equals("2")) {
                    List<Product> list = d.getProductByColor("White");
                    addProductToList(listA, list);

                }
                if (x.equals("3")) {
                    List<Product> list = d.getProductByColor("Gray");
                    addProductToList(listA, list);

                }
                if (x.equals("4")) {
                    List<Product> list = d.getProductByColor("Blue");
                    addProductToList(listA, list);
                }
                if (x.equals("5")) {
                   List<Product> list = d.getProductByColor("Red");
                    addProductToList(listA, list);
                }
                if (x.equals("6")) {
                    List<Product> list = d.getProductByColor("Green");
                    addProductToList(listA, list);
                }
